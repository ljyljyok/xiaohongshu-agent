...
def test_ollama_falls_back_from_chat_to_generate():
    ...
    with patch.object(client, "_preferred_endpoints", return_value=["/api/chat", "/api/generate", "/v1/chat/completions"]), \
         patch.object(client, "_post_json", side_effect=fake_post_json), \
         patch.object(client, "_post_openai_compatible", return_value={"choices": []}):
...
def test_ollama_falls_back_to_openai_compatible():
    client = OllamaClient(base_url="http://localhost:11434", default_model="gemma4:e4b")
    with patch.object(client, "_preferred_endpoints", return_value=["/api/chat", "/api/generate", "/v1/chat/completions"]), \
         patch.object(client, "_post_json", side_effect=lambda path, payload, timeout: (_ for _ in ()).throw(_http_error(404))), \
         patch.object(client, "_post_openai_compatible", return_value={"choices": [{"message": {"content": "ok-from-v1"}}]}):
        response = client.chat.completions.create(messages=[{"role": "user", "content": "hi"}])

    assert response.choices[0].message.content == "ok-from-v1"


def test_ollama_reports_endpoint_chain_on_total_failure():
    client = OllamaClient(base_url="http://localhost:11434", default_model="gemma4:e4b")
    with patch.object(client, "_preferred_endpoints", return_value=["/api/chat", "/api/generate", "/v1/chat/completions"]), \
         patch.object(client, "_post_json", side_effect=lambda path, payload, timeout: (_ for _ in ()).throw(_http_error(404))), \
         patch.object(client, "_post_openai_compatible", side_effect=RuntimeError("boom")):
        try:
...
def test_content_analyzer_uses_ollama_default_model():
